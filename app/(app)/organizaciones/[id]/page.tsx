...
import { Building2, Edit2, Calendar, Users, ArrowLeft, Plus } from 'lucide-react'
...
export default async function OrganizacionDetailPage({
  ...
  const [{ data: org, error }, { data: eventos }, { data: ministerios }] = await Promise.all([
    ...
    supabase
      ...
      .select('id, fecha_inicio, persona:personas(id, nombre, apellido), ministerio:ministerios(id, nombre, tipo)')
      .eq('organizacion_id', id)
      .is('fecha_fin', null),
  ])
  ...
  const parent = org.parent as { id: string; nombre: string; tipo: string } | null
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between">
        <div className="space-y-1">
          <Link href="/organizaciones" className="inline-flex items-center gap-2 text-sm text-primary hover:underline mb-2">
          ...
          <div className="flex items-center gap-3 mt-1">
            <span className="text-muted-foreground">{tipoLabel[org.tipo] ?? org.tipo}</span>
            {org.codigo && (
              <>
                <span className="text-muted-foreground">·</span>
                <span className="text-muted-foreground font-mono text-sm">{org.codigo}</span>
              </>
            )}
            <span className="text-muted-foreground">·</span>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="border-border bg-card">
          <CardHeader>
            <CardTitle className="text-foreground text-base">Información General</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            {parent && (
              <div className="flex justify-between">
                <span className="text-muted-foreground">Depende de</span>
                <Link href={`/organizaciones/${parent.id}`} className="text-primary hover:underline">
                  {parent.nombre} <span className="text-muted-foreground">({tipoLabel[parent.tipo] ?? parent.tipo})</span>
                </Link>
              </div>
            )}
            {org.localidad && (
        ...
        <Card className="border-border bg-card">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">{(ministerios ?? []).length} asignaciones</span>
          </CardHeader>
          <CardContent>
            {ministerios && ministerios.length > 0 ? (
              <ul className="space-y-2">
                {(ministerios as any[]).map(a => (
                  <li key={a.id} className="flex items-center justify-between text-sm">
                    <div>
                      <Link href={`/personas/${a.persona.id}`} className="text-foreground font-medium hover:text-primary">
                        {a.persona.apellido}, {a.persona.nombre}
                      </Link>
                      <p className="text-muted-foreground text-xs">{a.ministerio.nombre}</p>
                    </div>
              ...
              <p className="text-sm text-muted-foreground">No hay ministerios asignados actualmente.</p>
            )}
          </CardContent>
        </Card>
      </div>
                  ...
                  {proximosEventos.map(e => (
                    <tr key={e.id} className="border-b border-border hover:bg-muted/40">
                      <td className="py-2 px-3">
                        <Link href={`/eventos/${e.id}`} className="text-foreground hover:text-primary font-medium">
                          {e.nombre}
                        </Link>
                      </td>
                      <td className="py-2 px-3 text-muted-foreground">{tipoEventoLabel[e.tipo] ?? e.tipo}</td>
