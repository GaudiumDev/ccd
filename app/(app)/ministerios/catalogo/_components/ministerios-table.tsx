...
export function MinisteriosTable({
  ministerios: initial,
}: {
  ...
  const handleDeleteSelected = async () => {
    ...
    const conAsignaciones = ministerios.filter(
      (m) => selected.has(m.id) && m.asignaciones > 0,
    )
    if (conAsignaciones.length > 0) {
      setError(
        `No se pueden desactivar roles  con asignaciones activas: ${conAsignaciones.map((m) => m.nombre).join(", ")}`,
      )
      return
    }

    // Proteger admin_general
    const protegidos = ministerios.filter(
      (m) => selected.has(m.id) && m.nombre === "admin_general",
    )
    if (protegidos.length > 0) {
      setError(
        "El rol admin_general no se puede desactivar porque es requerido por el sistema de autenticación.",
      )
      return
    }
    ...
    if (err) {
      ...
      setMinisterios((prev) =>
        prev.map((m) => (selected.has(m.id) ? { ...m, activo: false } : m)),
      )
      setSelected(new Set())
    }
  ...
  const handleDeactivateOne = async (m: Ministerio) => {
    if (m.asignaciones > 0) {
      setError(
        `No se puede desactivar "${m.nombre}" porque tiene ${m.asignaciones} asignación${m.asignaciones !== 1 ? "es" : ""} activa${m.asignaciones !== 1 ? "s" : ""}.`,
      )
      return
    }
    ...
    if (err) {
      ...
      setMinisterios((prev) =>
        prev.map((r) => (r.id === m.id ? { ...r, activo: false } : r)),
      )
    }
    setDeletingId(null)
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {displayed.map((m) => {
              const nivel =
                nivelLabel[m.nivel_acceso] ??
                (m.nivel_acceso > 0
                  ? {
                      label: `Nivel ${m.nivel_acceso}`,
              ...
              return (
                ...
                >
                  ...
                  <td className="py-3 px-4 font-medium">
                    <Link
                      href={`/ministerios/catalogo/${m.id}`}
                  ...
                  <td className="py-3 px-4">
                    <span
                      className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${tipoColor[m.tipo] ?? "bg-muted text-muted-foreground"}`}
                    >
                  ...
                  <td className="py-3 px-4">
                    {nivel ? (
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${nivel.color}`}
                      >
                  ...
                  <td className="py-3 px-4 text-center">
                    <div className="inline-flex items-center gap-1">
                      ...
                      <Button
                        ...
                        title={
                          !m.activo ? "Ya está inactivo" : "Desactivar rol"
                        }
                      >
                        {deletingId === m.id ? (
